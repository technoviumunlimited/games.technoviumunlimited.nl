<template>
    <div class="hall-of-fame p-8">
      ...
      <div class="flex">
        <div class="w-1/4 pr-4 bg-gray-300 rounded-tl-lg rounded-bl-lg p-4">
          <!-- Navigation options go here -->
          <div class="mb-4">Frontenders</div>
          <div class="mb-4">Backenders</div>
          <div class="mb-4">Gamemakers</div>
          <!-- Add more options as needed -->
        </div>
        <div class="w-3/4 flex flex-wrap bg-white rounded-tr-lg rounded-br-lg p-4">
          <!-- Display people information here -->
          <div v-for="person in people" :key="person.id" class="w-1/4 mb-4 px-4">
            ...
            <!-- <img :src="person.logo" alt="Person Logo" class="max-w-full h-auto mb-2" /> -->
            <div class="text-center">
              <div class="font-bold text-lg">{{ person.name }}</div>
              <div class="text-gray-600">{{ person.position }}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    data() {
      return {
        people: [
          ...
          { id: 1, logo: '/path/to/logo1.png', name: 'John Doe', position: 'CEO' },
          { id: 2, logo: '/path/to/logo2.png', name: 'Jane Smith', position: 'CTO' },
